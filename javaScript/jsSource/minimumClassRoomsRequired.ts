...
*/
const minimumClassRoomsRequired = function (...classIntervals: number[][]): number {
  const startingIntervalTime: number[] = []
  const endingIntervalTime: number[] = []

  // 'numberOfmeetingInParallel' provides us the value of number of meeting happeing in parallel
  let numberOfmeetingInParallel: number = 0
